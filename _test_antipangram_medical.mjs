// Test AntiPangram engine on full medical AI paper — target: 0% AI, readable output
...
console.log('Testing AntiPangram engine on medical AI paper...\n');
...
try {
  const resp = await fetch('http://localhost:3000/api/humanize-stream', {
    ...
    body: JSON.stringify({ text, engine: 'antipangram', strength: 'strong', tone: 'academic' }),
  ...
  if (!doneLine) {
    console.error('No done event. Last 5 lines:');
  ...
  const checks = [
    ...
    ['Citations preserved', output.toLowerCase().includes('topol') && output.toLowerCase().includes('esteva')],
    ...
    ['No "bolster"', !/\bbolster/i.test(output)],
  ...
  if (score >= 90) {
    console.log('\n✅ PASS — AntiPangram output looks good!');
  } else {
    console.log('\n⚠️  NEEDS IMPROVEMENT');
  }
