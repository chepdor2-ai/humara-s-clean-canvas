...
async function testFull() {
  try {
    console.log('=== FULL KENYA EDUCATION TEXT TEST ===');
    const inputParagraphs = input.split(/\n\s*\n/).filter(p => p.trim());
    console.log(`Input: ${inputParagraphs.length} paragraphs, ${input.split(/\s+/).length} words\n`);
    ...
    console.log(`Output: ${outputParagraphs.length} paragraphs, ${completeOutput.split(/\s+/).length} words`);
    console.log(`Paragraph preservation: ${inputParagraphs.length === outputParagraphs.length ? '✓ PASS' : '✗ FAIL'}\n`);
    
    // Check for heading preservation
    const inputHeadings = inputParagraphs.filter(p => p.toLowerCase().match(/^(introduction|literature review)/));
    const outputHeadings = outputParagraphs.filter(p => p.toLowerCase().match(/^(introduction|literature review)/));
    console.log(`Heading preservation: Input=${inputHeadings.length}, Output=${outputHeadings.length}, ${inputHeadings.length === outputHeadings.length ? '✓' : '✗'}`);
    
    // Check for duplication  
    const fullText = completeOutput.toLowerCase();
    const introCount = (fullText.match(/nonprofit organizations play a decisive role/gi) || []).length;
    console.log(`Duplicate content check: '${introCount > 1 ? 'FOUND ' + introCount + ' duplicates ✗' : 'No major duplicates ✓'}\n`);
    
    // Show first portion
    console.log('Output excerpt:');
    console.log(outputParagraphs.slice(0, 2).map(p => p.substring(0, 120) + '...' ).join('\n\n'));
