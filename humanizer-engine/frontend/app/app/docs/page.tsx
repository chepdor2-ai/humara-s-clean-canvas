        ...
        <nav className="lg:w-52 shrink-0">
          <div className="lg:sticky lg:top-6 space-y-0.5">
            {SECTIONS.map(s => {
              const Icon = s.icon;
              return (
                <button key={s.id} onClick={() => setActiveSection(s.id)} className={`w-full text-left flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeSection === s.id ? 'bg-brand-50 dark:bg-brand-950 text-brand-700 dark:text-brand-300' : 'text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-800 hover:text-slate-900 dark:hover:text-white'}`}>
          ...
          {activeSection === 'overview' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
                ...
                <p className="text-sm text-slate-600 dark:text-zinc-400 leading-relaxed">HumaraGPT provides a REST API for humanizing AI-generated text. All requests require an API key passed via the <code className="bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-xs">Authorization</code> header.</p>
                <div className="flex items-start gap-3 p-4 bg-brand-50 dark:bg-brand-950/30 rounded-lg border border-brand-200 dark:border-brand-800">
                  <ChevronRight className="w-4 h-4 text-brand-600 mt-0.5 shrink-0" />
                  <p className="text-sm text-brand-800 dark:text-brand-300">Generate your API key from <a href="/app/settings" className="font-medium underline">Settings → API Keys</a>.</p>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 text-xs font-semibold rounded">POST</span>
                  <code className="text-sm font-mono text-slate-900 dark:text-white">/api/humanize</code>
                </div>
                <p className="text-sm text-slate-600 dark:text-zinc-400">Humanize AI-generated text.</p>
                <h3 className="text-sm font-semibold text-slate-800 dark:text-zinc-200">Request Body</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-slate-200 dark:border-zinc-700"><th className="text-left py-2 text-slate-500 dark:text-zinc-400 font-medium">Field</th><th className="text-left py-2 text-slate-500 dark:text-zinc-400 font-medium">Type</th><th className="text-left py-2 text-slate-500 dark:text-zinc-400 font-medium">Required</th><th className="text-left py-2 text-slate-500 dark:text-zinc-400 font-medium">Description</th></tr></thead>
                    <tbody className="text-slate-700 dark:text-zinc-300">
                      <tr className="border-b border-slate-100 dark:border-zinc-800"><td className="py-2 font-mono text-xs">text</td><td className="py-2">string</td><td className="py-2">Yes</td><td className="py-2">The text to humanize</td></tr>
                      <tr className="border-b border-slate-100 dark:border-zinc-800"><td className="py-2 font-mono text-xs">engine</td><td className="py-2">string</td><td className="py-2">No</td><td className="py-2">Engine ID (ghost_mini, ghost_pro, ninja, humara, fast_v11)</td></tr>
                      <tr className="border-b border-slate-100 dark:border-zinc-800"><td className="py-2 font-mono text-xs">strength</td><td className="py-2">string</td><td className="py-2">No</td><td className="py-2">light, medium, or strong</td></tr>
                ...
                <CodeBlock code={`// Example response
{
  ...
  "output_detector_results": {
    ...
    "detectors": [...]
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
                ...
                <p className="text-sm text-slate-600 dark:text-zinc-400">Detect AI-generated content in text.</p>
                <CodeBlock code={`{
  ...
  "detectors": ["gptzero", "turnitin", "originality"]
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 text-xs font-semibold rounded">GET</span>
                  <code className="text-sm font-mono text-slate-900 dark:text-white">/api/health</code>
                </div>
                <p className="text-sm text-slate-600 dark:text-zinc-400">Check API health and available engines.</p>
              </div>
            </div>
          )}
              ...
              {[
                ...
                <div key={e.id} className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${e.tier === 'Premium' ? 'bg-brand-100 dark:bg-brand-950 text-brand-700 dark:text-brand-300' : 'bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400'}`}>{e.tier}</span>
                    <code className="text-xs text-slate-400 font-mono">{e.id}</code>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-zinc-400 leading-relaxed mb-2">{e.desc}</p>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6">
                  ...
                  <p className="text-sm text-slate-600 dark:text-zinc-400 leading-relaxed">{tip.body}</p>
                </div>
              ))}
            </div>
          )}
