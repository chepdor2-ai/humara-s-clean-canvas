...
export default function DocumentsPage() {
  ...
  const scoreColor = (score: number | null) => {
    if (score === null) return 'text-slate-500 dark:text-zinc-500';
    ...
    <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-sm border border-slate-300 dark:border-zinc-700 rounded-lg bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 w-56"
            />
          </div>
          <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 dark:text-zinc-300 bg-slate-100 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-lg hover:bg-zinc-700 transition-colors">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800">
          <FileText className="w-12 h-12 text-slate-400 dark:text-zinc-600 mx-auto mb-4" />
          <p className="text-slate-500 dark:text-zinc-400 font-medium">No documents yet</p>
          <p className="text-sm text-slate-500 dark:text-zinc-500 mt-1">Humanized texts will appear here automatically.</p>
        </div>
      ) : (
        <>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-800/50">
                ...
                <tbody>
                  {filteredDocs.map((doc) => (
                    <tr key={doc.id} className="border-b border-slate-200 dark:border-zinc-800/50 hover:bg-slate-100 dark:bg-zinc-800/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <FileText className="w-4 h-4 text-slate-500 dark:text-zinc-500 shrink-0" />
                      ...
                      <td className="px-4 py-3 text-center text-slate-500 dark:text-zinc-400 hidden md:table-cell">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          ...
                          className="p-1.5 text-slate-500 dark:text-zinc-500 hover:text-red-400 rounded-lg hover:bg-red-900/20 transition-colors disabled:opacity-50"
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} className="p-2 rounded-lg border border-slate-300 dark:border-zinc-700 text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:bg-zinc-800 disabled:opacity-40 transition-colors">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="p-2 rounded-lg border border-slate-300 dark:border-zinc-700 text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:bg-zinc-800 disabled:opacity-40 transition-colors">
