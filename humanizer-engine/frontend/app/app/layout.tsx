...
const Logo = () => (
  <Link
    href="/"
    className="flex items-end text-2xl font-bold font-sora tracking-tight leading-none group mb-10 px-4"
  >
    <span className="text-[#5C4033]">Huma</span>
    <span className="text-[#D97757]">ra</span>
    <div className="w-1.5 h-1.5 rounded-full bg-[#7A8F6A] ml-1 mb-1 group-hover:scale-150 transition-transform" />
  </Link>
);
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { name: 'Advanced', href: '/app/advanced', icon: Activity },
  ...
  return (
    <div className="flex h-screen bg-[#FFF8F0] overflow-hidden">
          ...
          {links.map((link) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 ${
                    isActive ? 'text-[#D97757]' : 'text-[#8A7263]'
                  }`}
                />
                {link.name}
              </Link>
      ...
      <main className="flex-1 overflow-y-auto bg-[#FFF8F0] p-6">{children}</main>
    </div>
  );
}
