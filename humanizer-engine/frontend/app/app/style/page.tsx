    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">Create profiles so your writing always points to you.</p>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={resetForm} title="Close" className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2.5 text-sm border border-slate-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none"
            />
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800">
          <PenTool className="w-12 h-12 text-slate-300 dark:text-zinc-600 mx-auto mb-4" />
          <p className="text-slate-500 dark:text-zinc-400 font-medium">No style profiles yet</p>
          <p className="text-sm text-slate-400 dark:text-zinc-500 mt-1">Create your first profile to customize humanization output.</p>
        </div>
      ) : (
          ...
          {profiles.map(profile => (
            <div
              ...
              className={`group relative p-6 border rounded-xl transition-all ${
                profile.is_default
                  ? 'bg-brand-600 text-white border-brand-700 shadow-lg shadow-brand-200 dark:shadow-brand-900/30'
                  : 'bg-white dark:bg-zinc-900 text-slate-900 dark:text-white border-slate-200 dark:border-zinc-800 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-md'
              }`}
            >
              {/* Actions */}
              <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => startEdit(profile)} title="Edit profile" className={`p-1.5 rounded-lg transition-colors ${profile.is_default ? 'hover:bg-white/20 text-white/80' : 'hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-400'}`}>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {profile.is_default ? (
                  ...
                  <button
                    ...
                    className="text-xs font-medium px-2.5 py-1 rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400 hover:bg-brand-50 dark:hover:bg-brand-950 hover:text-brand-600 transition-colors"
                  >
              ...
              <p className={`text-sm leading-relaxed ${profile.is_default ? 'text-white/80' : 'text-slate-600 dark:text-zinc-400'}`}>
                {profile.description || 'No description set.'}
              </p>

              <div className={`mt-4 pt-3 border-t text-xs flex items-center gap-3 ${profile.is_default ? 'border-white/20 text-white/60' : 'border-slate-100 dark:border-zinc-800 text-slate-400'}`}>
