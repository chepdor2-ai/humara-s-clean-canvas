    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 dark:bg-zinc-800 rounded-lg w-fit">
        {[
          ...
          const Icon = tab.icon;
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === tab.id ? 'bg-white dark:bg-zinc-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:hover:text-zinc-300'}`}>
      ...
      {activeTab === 'tuning' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-800 rounded-lg border border-slate-200 dark:border-zinc-700 space-y-3">
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-zinc-400">
                <span>Estimated output profile:</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-white dark:bg-zinc-900 rounded-lg border border-slate-100 dark:border-zinc-700">
            ...
            <div className="p-4 bg-brand-50 dark:bg-brand-950/30 rounded-lg border border-brand-200 dark:border-brand-800">
              <p className="text-xs text-brand-700 dark:text-brand-300"><strong>Tip:</strong> These parameters are applied on top of the engine&apos;s defaults. Use the humanizer page to test with different settings.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'batch' && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
            ...
            {batchTexts.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm text-slate-600 dark:text-zinc-400">{batchTexts.length} document{batchTexts.length !== 1 ? 's' : ''} loaded</p>
                {batchTexts.map((t, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-zinc-800 rounded-lg border border-slate-200 dark:border-zinc-700">
                    <FileText className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-700 dark:text-zinc-300 truncate flex-1">Document {i + 1} — {t.split(/\s+/).length} words</span>
            ...
            {batchProcessing && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600 dark:text-zinc-400 flex items-center gap-2"><RotateCcw className="w-4 h-4 animate-spin" /> Processing...</span>
            ...
            <div className="flex items-center gap-3">
              ...
              {batchResults.length > 0 && (
                <button onClick={downloadResults} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 text-sm font-medium rounded-lg hover:bg-slate-50 dark:hover:bg-zinc-700 transition-colors">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <p className="text-sm text-slate-600 dark:text-zinc-400">Analytics dashboard is populated as you humanize more documents. Start processing to see trends.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-5 bg-slate-50 dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700">
