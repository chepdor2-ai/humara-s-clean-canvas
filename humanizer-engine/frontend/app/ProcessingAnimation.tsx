    ...
    <div className="flex flex-col items-center justify-center min-h-[380px] gap-6 px-8">
      ...
      <div className="relative">
        <div className="w-14 h-14 rounded-2xl bg-brand-50 dark:bg-brand-950 flex items-center justify-center">
          <Icon className="w-6 h-6 text-brand-600 dark:text-brand-400 animate-pulse" />
        </div>
        <div className="absolute -inset-3 rounded-3xl border-2 border-brand-200/50 dark:border-brand-700/40 animate-ping" />
      </div>
      ...
      {/* Progress bar */}
      <div className="w-52 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
        <div
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={i} className={`flex items-center gap-1 text-[10px] font-medium transition-all duration-300 ${
              i === step ? 'text-brand-600 dark:text-brand-400' : 'text-slate-300 dark:text-slate-600'
            }`}>
