...
-- Add check constraint for use_case if not exists
DO $$ BEGIN
  ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_use_case_check;
  ALTER TABLE public.profiles ADD CONSTRAINT profiles_use_case_check
    CHECK (use_case IN ('academic', 'content_seo', 'general') OR use_case IS NULL);
EXCEPTION WHEN others THEN NULL;
END $$;
...
DO $$ BEGIN
  ...
  ALTER TABLE public.subscriptions ADD COLUMN IF NOT EXISTS current_period_start TIMESTAMPTZ NOT NULL DEFAULT NOW();
  ALTER TABLE public.subscriptions ADD COLUMN IF NOT EXISTS current_period_end TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '30 days');
...
DO $$ BEGIN
  ALTER TABLE public.usage ADD COLUMN IF NOT EXISTS usage_date DATE NOT NULL DEFAULT CURRENT_DATE;
...
DO $$ BEGIN
  ALTER TABLE public.usage ADD CONSTRAINT usage_user_date_unique UNIQUE (user_id, usage_date);
...
DO $$ BEGIN
  ...
  ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS input_text TEXT NOT NULL DEFAULT '';
  ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS output_text TEXT;
  ...
  ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS premium BOOLEAN NOT NULL DEFAULT FALSE;
  ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'completed';
  ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
EXCEPTION WHEN others THEN NULL;
END $$;

-- Add/replace check constraints
DO $$ BEGIN
  ALTER TABLE public.documents DROP CONSTRAINT IF EXISTS documents_strength_check;
  ALTER TABLE public.documents ADD CONSTRAINT documents_strength_check
    CHECK (strength IN ('light','medium','strong') OR strength IS NULL);
EXCEPTION WHEN others THEN NULL;
END $$;

DO $$ BEGIN
  ALTER TABLE public.documents DROP CONSTRAINT IF EXISTS documents_tone_check;
  ALTER TABLE public.documents ADD CONSTRAINT documents_tone_check
    CHECK (tone IN ('natural','academic','business','direct') OR tone IS NULL);
EXCEPTION WHEN others THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS name TEXT NOT NULL DEFAULT '';
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS description TEXT DEFAULT '';
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS avg_sentence_length NUMERIC(6,2) NOT NULL DEFAULT 22.0;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS sentence_length_std NUMERIC(6,2) NOT NULL DEFAULT 8.0;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS hedging_rate NUMERIC(5,4) NOT NULL DEFAULT 0.18;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS clause_density NUMERIC(5,2) NOT NULL DEFAULT 1.4;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS passive_voice_rate NUMERIC(5,4) NOT NULL DEFAULT 0.20;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS lexical_diversity NUMERIC(5,4) NOT NULL DEFAULT 0.62;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS avg_paragraph_length NUMERIC(5,2) NOT NULL DEFAULT 4.5;
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS punctuation_rates JSONB NOT NULL DEFAULT '{"semicolons_per_1k":2.5,"colons_per_1k":1.8,"dashes_per_1k":1.2}';
  ALTER TABLE public.style_profiles ADD COLUMN IF NOT EXISTS is_default BOOLEAN NOT NULL DEFAULT FALSE;
...
DO $$ BEGIN
  ALTER TABLE public.api_keys ADD COLUMN IF NOT EXISTS name TEXT NOT NULL DEFAULT 'Default';
  ...
  ALTER TABLE public.api_keys ADD COLUMN IF NOT EXISTS requests INTEGER NOT NULL DEFAULT 0;
...
DO $$ BEGIN
  ALTER TABLE public.feedback ADD COLUMN IF NOT EXISTS document_id UUID;
...
DO $$ BEGIN
  ...
  ALTER TABLE public.feedback ADD CONSTRAINT feedback_category_check
    CHECK (category IN ('quality','speed','accuracy','bug','feature','other') OR category IS NULL);
EXCEPTION WHEN others THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.detection_cache ADD COLUMN IF NOT EXISTS text_hash TEXT;
  ALTER TABLE public.detection_cache ADD COLUMN IF NOT EXISTS word_count INTEGER NOT NULL DEFAULT 0;
  ALTER TABLE public.detection_cache ADD COLUMN IF NOT EXISTS overall_ai NUMERIC(5,2) NOT NULL DEFAULT 0;
  ALTER TABLE public.detection_cache ADD COLUMN IF NOT EXISTS overall_human NUMERIC(5,2) NOT NULL DEFAULT 0;
  ALTER TABLE public.detection_cache ADD COLUMN IF NOT EXISTS detectors JSONB NOT NULL DEFAULT '{}';
EXCEPTION WHEN others THEN NULL;
END $$;

-- Unique index on text_hash
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Users can view own style profiles" ON public.style_profiles;
DROP POLICY IF EXISTS "Users can insert own style profiles" ON public.style_profiles;
DROP POLICY IF EXISTS "Users can update own style profiles" ON public.style_profiles;
DROP POLICY IF EXISTS "Users can delete own style profiles" ON public.style_profiles;
...
DROP POLICY IF EXISTS "Authenticated can read cache" ON public.detection_cache;
DROP POLICY IF EXISTS "Authenticated can insert cache" ON public.detection_cache;
...
-- ============================================================
-- DONE — Migration is fully idempotent
-- ============================================================
