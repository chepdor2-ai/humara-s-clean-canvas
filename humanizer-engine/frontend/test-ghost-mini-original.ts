/**
 * Ghost Mini (Original) Engine Test
 * Tests the humanize() function with mode='ghost_mini' against all 22 detectors.
 * This is the ORIGINAL Ghost Mini pipeline (different from ghost-mini-v1-2).
 * Target: 15/22 detectors at 0% per essay.
 */
...
function runTest() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  GHOST MINI (Original) — Detector Score Test');
  console.log('  Target: 15/22 detectors at 0% per essay');
  console.log('═══════════════════════════════════════════════════════════\n');
  ...
  // Test with different strength/mode combos
  const configs = [
    { label: 'ghost_mini/medium/stealth', opts: { mode: 'ghost_mini', strength: 'medium', tone: 'academic', stealth: true, enablePostProcessing: true } },
    { label: 'ghost_mini/strong/stealth', opts: { mode: 'ghost_mini', strength: 'strong', tone: 'academic', stealth: true, enablePostProcessing: true } },
    { label: 'ghost_mini/medium/no-stealth', opts: { mode: 'ghost_mini', strength: 'medium', tone: 'academic', stealth: false, enablePostProcessing: true } },
  ];

  for (const config of configs) {
    console.log(`\n── CONFIG: ${config.label} ──\n`);
    let totalZeros = 0;
    let worstEssay = { title: '', zeros: 999, detectors: {} as Record<string, number> };

    for (const { title, text } of TEST_TEXTS) {
      const humanized = humanize(text, config.opts);
      const score = scoreText(humanized);
      totalZeros += score.zeros;
      console.log(`  ${title}: avg=${score.avg}% max=${score.max}% zeros=${score.zeros}/22`);
      if (score.zeros < worstEssay.zeros) {
        worstEssay = { title, zeros: score.zeros, detectors: score.detectors };
      }
    }
    console.log(`  TOTAL ZEROS: ${totalZeros}/${4*22}`);
    console.log(`  Target (15/22 per essay): ${totalZeros >= 60 ? 'MET ✓' : 'NOT MET ✗'}`);

    // Show per-detector for worst essay
    console.log(`\n  WORST ESSAY: "${worstEssay.title}" (zeros=${worstEssay.zeros}/22)`);
    const sorted = Object.entries(worstEssay.detectors).sort((a, b) => b[1] - a[1]);
    for (const [name, val] of sorted) {
      console.log(`    ${name.padEnd(24)} ${val === 0 ? '✓ 0%' : val + '%'}`);
    }
  }

  // Detailed output for best config on essay 1
  console.log('\n\n── DETAILED OUTPUT (ghost_mini/medium/stealth, Essay 1) ──\n');
  const humanized1 = humanize(essay1.text, { mode: 'ghost_mini', strength: 'medium', tone: 'academic', stealth: true, enablePostProcessing: true });
  const outputScore = scoreText(humanized1, true);
  console.log('  OUTPUT signals:');
  ...
  console.log(`\n  Preview: "${humanized1.substring(0, 300).replace(/\n/g, ' ')}..."`);
}
