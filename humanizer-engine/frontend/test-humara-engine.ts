/**
 * Humara Engine Test
 * Tests the Humara humanizer against all 22 detectors.
...
function runTest() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  HUMARA ENGINE — Detector Score Test');
  ...
  // Test with different strength/tone combos
  const configs = [
    { label: 'medium/academic', opts: { strength: 'medium' as const, tone: 'academic' as const, strictMeaning: false } },
    { label: 'heavy/academic', opts: { strength: 'heavy' as const, tone: 'academic' as const, strictMeaning: false } },
    { label: 'medium/neutral', opts: { strength: 'medium' as const, tone: 'neutral' as const, strictMeaning: false } },
  ];
  ...
  for (const config of configs) {
    ...
    for (const { title, text } of TEST_TEXTS) {
      const humanized = humaraHumanize(text, config.opts);
  ...
  console.log('\n\n── DETAILED OUTPUT (medium/academic, Essay 1) ──\n');
  const humanized1 = humaraHumanize(essay1.text, { strength: 'medium', tone: 'academic', strictMeaning: false });
