...
const engines = ['ninja', 'omega', 'ghost_pro', 'ghost_mini', 'ghost_mini_v1_2', 'nuru', 'humara', 'humara_v1_3', 'fast_v11', 'undetectable'];
...
async function testEngine(engine) {
  try {
    ...
    if (d.error) {
      console.log(`  ${engine.padEnd(18)} | ERROR: ${d.error}`);
      return null;
    }
    ...
    console.log(`  ${engine.padEnd(18)} | AI: ${String(ai + '%').padEnd(8)} | Meaning: ${String(meaning).padEnd(5)} | Words: ${words}/${inputWords} | Time: ${elapsed}s`);
    
    // Print first 200 chars of output for quality check
    const preview = (d.humanizedText || d.output || '').substring(0, 250);
    console.log(`    Preview: ${preview}...`);
    console.log('');
    return { engine, ai, meaning, words, elapsed };
  } catch (e) {
    console.log(`  ${engine.padEnd(18)} | FETCH ERROR: ${e.message}`);
    return null;
  }
}

console.log(`\n=== ALL-ENGINE TEST (Cybersecurity Text) ===`);
console.log(`Input: ${inputWords} words\n`);
...
(async () => {
  ...
  console.log('\n=== SUMMARY ===');
  console.log('Engine'.padEnd(18) + ' | AI%'.padEnd(10) + ' | Words');
  console.log('-'.repeat(50));
  for (const r of results) {
    console.log(`${r.engine.padEnd(18)} | ${String(r.ai + '%').padEnd(10)} | ${r.words}/${inputWords}`);
  }
})();
