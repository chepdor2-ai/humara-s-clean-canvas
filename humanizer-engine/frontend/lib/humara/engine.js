...
function injectDiscourseMarker(sent) {
  ...
  // ~12% chance to inject (reduced to avoid cluttering)
  if (Math.random() > 0.12) return sent;
...
function processSentence(sent, style, aggr, usedSet, keywords, sentIndex, sentCount) {
  ...
  // 90% of sentences get restructured; first sentence too — restructuring before SYN preserves patterns
  const isFirstInPara = sentIndex === 0;
  const restructureProb = 0.90;
