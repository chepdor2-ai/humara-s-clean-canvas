...
async function groqCall(
  ...
  try {
    ...
    console.warn(`[Premium] Groq fallback failed: ${err instanceof Error ? err.message : String(err)}`);
    throw err;
  }
}
...
export async function premiumHumanize(
  ...
  // ═══════════════════════════════════════════
  // AGGRESSIVE NON-LLM AI SIGNAL ELIMINATION
  // Mirrors Omega's 9-phase post-processing pipeline
  // ═══════════════════════════════════════════
  console.log("  [Premium] Running aggressive non-LLM AI signal elimination...");

  // Phase 1: Kill AI words (regex-based, catches what LLM Phase B missed)
  result = applyAIWordKill(result);

  // Phase 2: Kill AI phrases
  result = applyPhrasePatterns(result);

  // Phase 3: Naturalize formal connectors
  result = applyConnectorNaturalization(result);

  // Phase 4: Diversify sentence starters
  result = diversifyStarters(result);

  // Phase 5: Expand ALL remaining contractions
  if (!features.hasContractions) {
    result = expandAllContractions(result);
    result = expandContractions(result);
  }

  // Phase 6: Per-sentence anti-detection pass
  {
    const ppSentences = robustSentenceSplit(result);
    const antiDetected = perSentenceAntiDetection(ppSentences, features.hasContractions);
    const deepCleaned = deepCleaningPass(antiDetected);
    result = deepCleaned.join(" ");
  }

  // Phase 7: Second AI word sweep (catch anything generated by earlier phases)
  result = applyAIWordKill(result);

  // Phase 8: Fix basic punctuation
  ...
  try {
    ...
    for (let feedbackRound = 0; feedbackRound < 2; feedbackRound++) {
      const detection = feedbackDetector.analyze(result);
