...
const ORIGINALITY_PREDICTABLE: Array<{ match: RegExp; rep: string }> = [
  ...
  // Extended Originality.ai specific signals
  { match: /\bencapsulat(?:e|es|ed|ing)\b/gi, rep: 'capture' },
  { match: /\belucidat(?:e|es|ed|ing)\b/gi, rep: 'explain' },
  { match: /\billuminat(?:e|es|ed|ing)\b/gi, rep: 'show' },
  { match: /\bexemplif(?:y|ies|ied|ying)\b/gi, rep: 'show' },
  { match: /\bembod(?:y|ies|ied|ying)\b/gi, rep: 'represent' },
  { match: /\bressonat(?:e|es|ed|ing)\b/gi, rep: 'connect' },
  { match: /\bresonat(?:e|es|ed|ing)\b/gi, rep: 'connect' },
  { match: /\bunderscor(?:e|es|ed|ing)\b/gi, rep: 'highlight' },
  { match: /\bsynergistic(?:ally)?\b/gi, rep: 'combined' },
  { match: /\bmultifaceted\b/gi, rep: 'complex' },
  { match: /\bseminal\b/gi, rep: 'influential' },
  { match: /\bburgeo(?:n|ns|ned|ning)\b/gi, rep: 'grow' },
  { match: /\bnascent\b/gi, rep: 'emerging' },
  { match: /\bvibrant\b/gi, rep: 'active' },
  { match: /\bparadigm shift\b/gi, rep: 'major shift' },
  { match: /\bgame[- ]changer?\b/gi, rep: 'turning point' },
  { match: /\bgame[- ]changing\b/gi, rep: 'major' },
  { match: /\bit can be argued (?:that)?\b/gi, rep: 'arguably, ' },
  { match: /\bmoving forward[,]?\s*/gi, rep: '' },
  { match: /\bgoing forward[,]?\s*/gi, rep: '' },
  { match: /\bin recent years[,]?\s*/gi, rep: 'recently, ' },
  { match: /\bbest practices?\b/gi, rep: 'standard methods' },
  { match: /\bhas emerged as (?:a|an)\b/gi, rep: 'has become a' },
  { match: /\bcan be attributed to\b/gi, rep: 'stems from' },
  { match: /\bholistic (?:approach|understanding|view|perspective|framework)\b/gi, rep: 'broad $1' },
  { match: /\bever[- ]evolving\b/gi, rep: 'changing' },
  { match: /\bever[- ]growing\b/gi, rep: 'growing' },
  { match: /\bever[- ]changing\b/gi, rep: 'changing' },
  { match: /\bwithout doubt\b/gi, rep: 'clearly' },
  { match: /\bwithout question\b/gi, rep: 'clearly' },
  { match: /\bundeniably\b/gi, rep: 'clearly' },
  { match: /\bindubitably\b/gi, rep: 'clearly' },
  { match: /\btime[- ]honored\b/gi, rep: 'long-established' },
  { match: /\brich (?:history|tradition|heritage|tapestry)\b/gi, rep: 'long $1' },
  { match: /\bprofound (?:impact|effect|influence|implications?)\b/gi, rep: 'strong $1' },
  { match: /\bpivotal (?:role|moment|point|shift|step|change)\b/gi, rep: 'key $1' },
  { match: /\bgroundbreaking (?:research|work|study|approach|solution|technology)\b/gi, rep: 'new $1' },
  { match: /\bunprecedented (?:growth|success|rate|scale|level|pace)\b/gi, rep: 'remarkable $1' },
];
...
const TURNITIN_FLUFF: Array<{ match: RegExp; rep: string }> = [
  ...
  // Additional Turnitin patterns
  { match: /\bit (?:has been|is) (?:widely|generally|commonly) (?:established|demonstrated|shown|proven) that\b/gi, rep: 'most agree that' },
  { match: /\bit can be argued that\b/gi, rep: 'arguably,' },
  { match: /\bone could (?:argue|suggest|say) that\b/gi, rep: 'arguably,' },
  { match: /\bit is (?:important|essential|crucial|vital|necessary) (?:to note|to consider|to recognize|to understand|to acknowledge) that\b/gi, rep: 'notably,' },
  { match: /\bplays? a (?:crucial|vital|key|significant|important|major|central|fundamental|pivotal) role in\b/gi, rep: 'is central to' },
  { match: /\bserves? as (?:a|an) (?:crucial|vital|key|important|essential|critical) (?:tool|mechanism|framework|foundation|component)\b/gi, rep: 'works as' },
  { match: /\bhas (?:profound|significant|important|major|far-reaching) implications for\b/gi, rep: 'matters for' },
  { match: /\brepresents? (?:a|an) (?:significant|major|important|key|fundamental) (?:step|shift|change|milestone)\b/gi, rep: 'marks a' },
  { match: /\bthe (?:significance|importance|value|impact) of .{3,30}? cannot be (?:overstated|underestimated|overlooked)\b/gi, rep: 'this deserves attention' },
  { match: /\bas (?:previously|earlier) (?:mentioned|noted|discussed|stated|outlined)\b/gi, rep: 'as noted' },
  { match: /\ba (?:growing|increasing|mounting) body of (?:research|evidence|literature|work) (?:suggests?|shows?|indicates?|demonstrates?)\b/gi, rep: 'more research shows' },
  { match: /\bin the (?:realm|sphere|domain|landscape|arena|world) of\b/gi, rep: 'in the area of' },
  { match: /\bhas (?:gained|garnered|attracted|received) (?:significant|considerable|widespread|substantial|growing) (?:attention|interest|traction|recognition)\b/gi, rep: 'has drawn attention' },
  { match: /\bhas emerged as (?:a|an) (?:key|crucial|vital|leading|prominent|primary|dominant)\b/gi, rep: 'has become a' },
  { match: /\bdemonstrates? (?:a|an) (?:commitment|dedication|focus) to\b/gi, rep: 'shows commitment to' },
  { match: /\bfoster(?:s|ed|ing)? (?:a|an) (?:sense|culture|environment) of\b/gi, rep: 'build a sense of' },
  { match: /\bprovide(?:s|d)? (?:a|an) (?:comprehensive|holistic|thorough|detailed|in-depth) (?:overview|analysis|understanding|framework|examination)\b/gi, rep: 'give a full $5' },
];
...
const COPYLEAKS_HEDGES: Array<{ match: RegExp; rep: string }> = [
  ...
  // Additional Copyleaks hedge patterns
  { match: /\bit can be argued that\b/gi, rep: 'arguably,' },
  { match: /\bone might (?:argue|say|suggest) that\b/gi, rep: 'arguably,' },
  { match: /\bit is worth (?:noting|mentioning|considering|emphasizing) that\b/gi, rep: 'notably,' },
  { match: /\bthis is particularly important (?:because|as|since)\b/gi, rep: 'this matters because' },
  { match: /\bthis is especially important (?:in|for|when)\b/gi, rep: 'this matters especially in' },
  { match: /\bit (?:is|becomes) (?:clear|evident|apparent|obvious) that\b/gi, rep: 'clearly,' },
  { match: /\bthere is no denying that\b/gi, rep: 'clearly,' },
  { match: /\bone cannot (?:deny|ignore|overlook) that\b/gi, rep: 'clearly,' },
  { match: /\bit is safe to say that\b/gi, rep: '' },
  { match: /\bit is fair to say that\b/gi, rep: '' },
  { match: /\bsuffice it to say,?\s*/gi, rep: '' },
];
...
const WINSTON_FLUFF: Array<{ match: RegExp; rep: string }> = [
  ...
  // Additional Winston patterns
  { match: /\bhas become increasingly (?:important|relevant|critical|essential|vital|necessary)\b/gi, rep: 'matters more' },
  { match: /\bburgeo(?:n|ns|ned|ning|ing) (?:field|industry|area|sector|market|space)\b/gi, rep: 'growing $1' },
  { match: /\bnascent (?:technology|technologies|field|industry|area)\b/gi, rep: 'new $1' },
  { match: /\bin recent years,?\s+/gi, rep: 'Recently, ' },
  { match: /\bhas (?:become|grown into) (?:a|an) increasingly (?:popular|common|widespread|important|prominent)\b/gi, rep: 'is now a popular' },
  { match: /\bit is (?:undeniable|undeniably true) that\b/gi, rep: 'clearly,' },
  { match: /\bwithout (?:doubt|question) that\b/gi, rep: 'clearly,' },
  { match: /\bfirst and foremost,?\s*/gi, rep: 'first, ' },
  { match: /\bat first glance,?\s*/gi, rep: 'on the surface, ' },
  { match: /\bat the end of the day,?\s*/gi, rep: '' },
  { match: /\bthe bottom line is\b/gi, rep: 'the key point is' },
];
...
export function universalPhraseSweep(text: string): string {
  ...
  const sweepRules: Array<{ pattern: RegExp; rep: string }> = [
    ...
    // Additional universal sweep patterns targeting all detectors
    { pattern: /\bin recent years\b/gi, rep: 'lately' },
    { pattern: /\bbest practices?\b/gi, rep: 'standard methods' },
    { pattern: /\bparadigm shift\b/gi, rep: 'major shift' },
    { pattern: /\bgame[- ]changer?\b/gi, rep: 'major development' },
    { pattern: /\bhas emerged as (?:a|an)\b/gi, rep: 'has become a' },
    { pattern: /\bcutting[- ]edge\b/gi, rep: 'advanced' },
    { pattern: /\bstate[- ]of[- ]the[- ]art\b/gi, rep: 'advanced' },
    { pattern: /\bit can be argued\b/gi, rep: 'arguably' },
    { pattern: /\bneedless to say[,]?\s*/gi, rep: '' },
    { pattern: /\bat the (?:heart|core) of\b/gi, rep: 'central to' },
    { pattern: /\bat the forefront of\b/gi, rep: 'leading in' },
    { pattern: /\bever[- ]evolving\b/gi, rep: 'changing' },
    { pattern: /\bvibrant (?:community|ecosystem|culture|landscape|environment)\b/gi, rep: 'active $1' },
    { pattern: /\bburgeoning\b/gi, rep: 'growing' },
    { pattern: /\bnascent\b/gi, rep: 'emerging' },
    { pattern: /\bseminal\b/gi, rep: 'influential' },
    { pattern: /\bmultifaceted\b/gi, rep: 'complex' },
    { pattern: /\bsynergistic\b/gi, rep: 'combined' },
    { pattern: /\bseamless(?:ly)?\b/gi, rep: 'smooth' },
    { pattern: /\bpivotal\b/gi, rep: 'key' },
    { pattern: /\brobust (?:approach|solution|framework|system|model|methodology)\b/gi, rep: 'solid $1' },
    { pattern: /\binnovative (?:approach|solution|method|framework|way|strategy)\b/gi, rep: 'new $1' },
    { pattern: /\bgroundbreaking\b/gi, rep: 'new' },
    { pattern: /\btransformative\b/gi, rep: 'major' },
    { pattern: /\bunprecedented\b/gi, rep: 'unusual' },
    { pattern: /\bfirst and foremost,?\s*/gi, rep: '' },
    { pattern: /\bit is worth noting that\s*/gi, rep: 'notably, ' },
    { pattern: /\bit should be noted that\s*/gi, rep: 'notably, ' },
    { pattern: /\bwith this in mind,?\s*/gi, rep: '' },
    { pattern: /\bas (?:mentioned|noted|discussed) (?:above|previously|earlier|before),?\s*/gi, rep: '' },
    { pattern: /\bto this end,?\s*/gi, rep: '' },
    { pattern: /\bfor this reason,?\s*/gi, rep: 'so ' },
    { pattern: /\bin this context,?\s*/gi, rep: '' },
  ];
