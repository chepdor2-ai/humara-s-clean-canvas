...
export async function t5Humanize(
  ...
  if (T5_BACKUP_URL) {
    try {
      ...
      console.warn(`[T5] Backup also failed: ${backupErr instanceof Error ? backupErr.message : backupErr}`);
    }
  }

  // Phase 3: Oxygen TS fallback — instant, serverless, always available
  console.log('[T5] All API endpoints failed, falling back to Oxygen TS engine');
